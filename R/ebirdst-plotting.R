...
plot_pds <- function(path, pds, predictor, ext,
                     ...
                     ylim = NULL,
                     plot = TRUE) {
  stopifnot(is.data.frame(pds), "covariate" %in% names(pds))
  ...
  predictor_label <- these_predictors$predictor_label[predictor_raw]
  predictor_raw <- these_predictors$predictor[predictor_raw]

  # subset pd
  pds <- ebirdst_subset(pds, ext = ext)
  pds <- pds[pds$covariate == predictor_raw, ]

  # pick occ or abd pis
  model <- match.arg(model)
  if (model == "occurrence") {
    pds <- pds[pds$model == "occ", ]
  } else {
    pds <- pds[pds$model == "abd", ]
  }

  n_preds <- 25
  pd_long <- pds[, c("stixel_id", "predictor_value", "response")]
  names(pd_long) <- c("stixel_id", "x", "y")
  rm(pds)
